...
def train(args):
    data_loader = get_loader(args.img_dir, args.latex_dir, vocab, args.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    for epoch in range(args.num_epochs):
        for i, (images, latex_markups, lengths) in enumerate(data_loader):

            # Set mini-batch dataset
            images = to_var(images, volatile=True)
            latex_markups = to_var(latex_markups)
            targets = pack_padded_sequence(latex_markups, lengths, batch_first=True)[0]
